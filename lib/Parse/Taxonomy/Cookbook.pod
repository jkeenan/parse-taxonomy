...
We're now in a position to create a B<new> Parse::Taxonomy::Path object based
on the data extracted from the database via the CSV file.  We'll use the
C<components> interface to C<Parse::Taxonomy::Path::new()> because our
taxonomy now lives in the form of Perl data structures suitable for the
C<components> interface.

    $ptpobj = Parse::Taxonomy::Path->new( {
        components  => {
            fields          => $fields,
...
source for a second Parse::Taxonomy::Path object.  Remember that C<--> was
used to separate the parts of the materialized path in that file.

    $tax = Parse::Taxonomy::Path->new( {
        file => '/path/to/mu.csv,
        path_col_sep => '--',
    } );
